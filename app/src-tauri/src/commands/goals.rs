...
pub fn create_goal(state: State<DbState>, data: CreateGoal) -> Result<Goal, String> {
    ...
    conn.execute(
        ...
        rusqlite::params![data.title, data.description, data.skill_id, data.target_date],
    )
    .map_err(|e| e.to_string())?;
...
pub fn update_goal(
    ...
    if let Some(v) = title {
        conn.execute("UPDATE goals SET title = ?1 WHERE id = ?2", rusqlite::params![v, id]).map_err(|e| e.to_string())?;
    }
    if let Some(v) = description {
        conn.execute("UPDATE goals SET description = ?1 WHERE id = ?2", rusqlite::params![v, id]).map_err(|e| e.to_string())?;
    }
    if let Some(v) = status {
        conn.execute("UPDATE goals SET status = ?1 WHERE id = ?2", rusqlite::params![v, id]).map_err(|e| e.to_string())?;
    }
    if let Some(v) = target_date {
        conn.execute("UPDATE goals SET target_date = ?1 WHERE id = ?2", rusqlite::params![v, id]).map_err(|e| e.to_string())?;
    }
    if let Some(v) = skill_id {
        conn.execute("UPDATE goals SET skill_id = ?1 WHERE id = ?2", rusqlite::params![v, id]).map_err(|e| e.to_string())?;
    }
