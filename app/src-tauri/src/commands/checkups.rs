...
use crate::models::{CreateHealthCheckup, HealthCheckup};
use tauri::State;

#[tauri::command]
pub fn list_health_checkups(
    state: State<DbState>,
    category: Option<String>,
) -> Result<Vec<HealthCheckup>, String> {
...
#[tauri::command]
pub fn create_health_checkup(
    state: State<DbState>,
    data: CreateHealthCheckup,
) -> Result<HealthCheckup, String> {
    let conn = state.0.lock().map_err(|e| e.to_string())?;
    let checkup_date = data
        .checkup_date
...
pub fn update_health_checkup(
    ...
    if let Some(v) = title {
        conn.execute(
            "UPDATE health_checkups SET title = ?1 WHERE id = ?2",
            rusqlite::params![v, id],
        )
        .map_err(|e| e.to_string())?;
    }
    if let Some(v) = checkup_date {
        conn.execute(
            "UPDATE health_checkups SET checkup_date = ?1 WHERE id = ?2",
            rusqlite::params![v, id],
        )
        .map_err(|e| e.to_string())?;
    }
    if let Some(v) = provider {
        conn.execute(
            "UPDATE health_checkups SET provider = ?1 WHERE id = ?2",
            rusqlite::params![v, id],
        )
        .map_err(|e| e.to_string())?;
    }
    if let Some(v) = category {
        conn.execute(
            "UPDATE health_checkups SET category = ?1 WHERE id = ?2",
            rusqlite::params![v, id],
        )
        .map_err(|e| e.to_string())?;
    }
    if let Some(v) = results {
        conn.execute(
            "UPDATE health_checkups SET results = ?1 WHERE id = ?2",
            rusqlite::params![v, id],
        )
        .map_err(|e| e.to_string())?;
    }
    if let Some(v) = notes {
        conn.execute(
            "UPDATE health_checkups SET notes = ?1 WHERE id = ?2",
            rusqlite::params![v, id],
        )
        .map_err(|e| e.to_string())?;
    }
...
pub fn delete_health_checkup(state: State<DbState>, id: i64) -> Result<(), String> {
    let conn = state.0.lock().map_err(|e| e.to_string())?;
    conn.execute("DELETE FROM health_checkups WHERE id = ?1", [id])
        .map_err(|e| e.to_string())?;
    Ok(())
}
